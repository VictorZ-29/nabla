...
- **`PROGRAMME.md`** is canonical for **official programme coverage**: which chapters each level/subject requires, each chapter's official scope, and what is already live. Read the relevant row before creating or scoping any chapter; update its status in the same commit that ships a chapter.
