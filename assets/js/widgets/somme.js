/* Nabla — widgets/somme.js
   ...
   cases (la somme 1 + 2 + ⋯ + n), et un bouton qui ajoute le même escalier
   tourné de 180° — sa colonne pleine à gauche, tête-bêche : ensemble ils
   remplissent exactement un rectangle de n lignes sur (n + 1) colonnes.
   D'où n(n+1)/2. Curseur n, lecture en direct, étiquettes des dimensions
   quand le second escalier est posé.
...
function initSomme(fig) {
  ...
  function dessiner() {
    ...
    /* escalier : colonne c (1 … n) de hauteur c — la première colonne reste
       libre pour la copie ; copie : le même escalier tourné de 180°, colonne
       pleine à GAUCHE au-dessus de la petite marche (vraie tête-bêche),
       colonne c de hauteur n − c posée depuis le haut */
    for (let c = 1; c <= n; c++) {
    ...
    for (let c = 0; c <= n; c++) {
      for (let rang = c; rang < n; rang++) case_(gCopie, c, rang);
    }
  ...
  function rendreEtat() {
    ...
    toggle.textContent = copieVisible ? '↺ retire l’escalier tête-bêche' : '▸ ajoute l’escalier tête-bêche';
