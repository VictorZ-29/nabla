...
|---|---|---|
| `s1` | 1. À quelle vitesse, exactement ? | hook prose · static sécante figure · display formula (taux) |
| `s2` | 2. Resserre les deux points | **widget sécante** · DÉFINITION · display formula (limite) · MÉTHODE (calculer f′(a)) · À RETENIR |
| `s3` | 3. La tangente à une courbe | PROPRIÉTÉ (équation) · **widget tangente** · MÉTHODE (équation d'une tangente) |
| `s4` | 4. Dérivées usuelles et opérations | table DÉRIVÉES USUELLES · table OPÉRATIONS · MÉTHODE (dériver en pratique) · **widget construis f′** |
| `s5` | 5. Signe de f′ et variations | PROPRIÉTÉ · TABLEAU DE VARIATIONS · MÉTHODE (étudier les variations) |
| `pieges` | Les pièges classiques | 4 pièges |
| `ex` | Exercices — 15 corrigés | ex. 01–03 with corrigés · placeholder « Exercices 4 à 14 » · **VERS LE BAC** (ex. 15, `#bac`) |
...
| Event | Props | Fired |
|---|---|---|
| `widget_interact` | `widget: secante \| tangente \| derivee`, `chapitre: derivation` | first interaction, then ≤ 1 / 30 s per widget |
| `corrige_open` | `exercice: 01 \| 02 \| 03`, `chapitre: derivation` | when a corrigé is opened |
